...
export async function updateRequestCompleted(
  requestId: number,
): Promise<boolean> {
  ...
  try {
    const { data, error } = await supabase
      ...
      .eq("id", requestId);
    if (error) {
      console.log("Error in updateRequestCompleted:", error);
      return false;
    }
    return true;
  } catch (error) {
    console.log("Unexpected error in updateRequestCompleted:", error);
    throw error;
  }
}
