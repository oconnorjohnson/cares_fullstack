...
export default function NewsCardThree({
  ...
  newsCardThreeData: NewsCard;
}) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const cardTitle = newsCardThreeData.card_title;
  const cardDescription = newsCardThreeData.card_description;
  const cardContent = newsCardThreeData.card_content;
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast.success("News Card Three updated successfully");
    } catch (error) {
      console.error("Error updating news card three:", error);
      toast.error("Error updating news card threes");
    } finally {
    ...
    <Card className="w-2/3 h-[350px] flex flex-col justify-between">
      ...
      <CardFooter className="py-4">
        <Dialog>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Edit News Card #3</DialogTitle>
              <DialogDescription>
